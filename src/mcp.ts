...
7. Later, when returning to a project that already has an identityforge.json. Start with check_applied_theme({dir}), which reads the stamp for you and answers three separate questions in one call: whether the KIT moved to a new version, whether only the rendered DESIGN.md bytes moved (a serializer change, not a design change, and not a reason to touch code), and whether the document's SHAPE moved, meaning a section was added, renamed or removed. It includes the diff when the kit did move, and it hashes the files the last apply wrote against what is on disk, so a hand-edited DESIGN.md is named before anything overwrites it. Reach for diff_kit_versions({slug: <id>, from: <stamped version>}) directly only when you are asking about a kit this project was not built against, or comparing two versions neither of which is the stamped one. list_kit_versions shows the version timeline with the author's note for each version, and get_kit_version returns a whole past snapshot when you need the old values themselves. list_kit_history is the wider record of the same kit: it adds every time the kit was applied to a brand, which mints no version and so appears in no timeline, and get_kit_history_event returns the kit as it stood at one of those entries. Reach for it when the question is whether a kit was ever actually used rather than merely edited. Only kits saved under an API key accumulate versions: a curated catalog kit is shipped rather than edited, so its timeline is empty and its version stays 0. The same three exist for brand projects as list_brand_project_versions, get_brand_project_version and diff_brand_project_versions, and they record the whole brand: its name and domain, its fonts, its pinned layers, its context, and its variations. A project that has not been written since versioning was wired has an empty timeline because the record starts there, not because nothing happened to it. All eight are read-only; none of them restores anything, and putting an old state back is an update_theme call you make deliberately.
...
4. Use search_name_evidence for model-authored exact-name, market, meaning, language, negative-association, or official-register discovery queries. It returns bounded SearXNG results without classifying them. Use check_domains for separate DNS, RDAP, and optional domain-SERP evidence. Attach raw evidence plus your interpretation to candidates; neither tool accepts, rejects, ranks, legally clears, or guarantees a purchase.
...
6. Visit material collision sources. Before selection, call search_trademarks for the EUIPO verbal-element screen in the relevant Nice classes, then visit other relevant official registers and check the chosen domain with an accredited registrar. The tool uses a shared quota and reports when to retry; it is an initial screen, not legal clearance. Record jurisdiction, query, classes, source URLs, and checkedAt.
7. A project can have only one selected candidate. Selecting it also updates the browser-facing chosen brand name.

Scopes & quota: naming keys need naming:read for reads/domain research and naming:write for projects, generations, and board edits; kits:write covers creating kits/brands. Generation spends AI credits per successfully persisted unique name; ordinary API calls use the separate monthly API-call quota. Full agent docs: https://identityforge.io/for-agents.`
...
export function buildMcpServer(): McpServer {
		...
		"list_themes",
		{
			...
			inputSchema: {
				...
				sort: z
					...
					.optional()
					.describe(
						"Order: featured (default, curated) | popular (most saved+installed) | recent | name | fit (computed use-case fitness; needs use=; re-orders rather than narrows).",
					),
				q: z
					.string()
					.optional()
	...
	server.registerTool(
		"get_image_direction",
		{
			...
			description:
				"Retrieve one image direction's full implementation export, as Markdown for reading and briefing or JSON for programmatic use. Follow it when generating, sourcing, or art-directing imagery, and adapt it into the project's repeatable production brief. A supplied reference subject stays recognizable; apply the direction to the presentation around it, then give that brief to the user's image model. This pairs with the design kit rather than replacing any of it. Read-only: it returns the text and writes nothing to disk. Free records are public; a Pro record returns 403 with an upgrade path unless the key is entitled, without leaking the payload.",
			inputSchema: {
				slug: z
					.string()
					.min(1)
	...
	server.registerTool(
		...
		async ({ slug, limit, before }) => {
			try {
				...
				if (data.length === 0) {
					return textResult(
						`"${slug}" has no recorded versions. Its current version is ${meta.currentVersion}. Catalog kits are shipped rather than edited, so only kits saved under your own key accumulate history.`,
					)
				}
		...
		"diff_kit_versions",
		{
			...
			inputSchema: {
				...
				from: z
					...
					.optional()
					.describe(
						"Lower bound. Omit to diff the current version against the one before it. Must be lower than the upper bound.",
					),
				to: z
					.number()
					.int()
	...
	server.registerTool(
		...
		async ({ projectId, limit, before }) => {
			try {
				...
				if (data.length === 0) {
					return textResult(
						`Project ${projectId} has no recorded versions. Its current version is ${meta.currentVersion}. Variation edits are not recorded on this timeline, so this is expected unless the project's brand name has been set or cleared.`,
					)
				}
				return textResult(
					`${
		...
		"diff_brand_project_versions",
		{
			...
			inputSchema: {
				...
				from: z
					...
					.min(0)
					.optional()
					.describe("Lower bound. Must be lower than the upper bound."),
				to: z
					.number()
					.int()
					.min(1)
