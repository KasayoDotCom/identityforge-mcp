...
Image directions, interface styles, and page recipes are separate collections that answer questions a kit does not. An image direction decides how imagery gets made, an interface style is a neutral render grammar for surfaces and hierarchy, and a page recipe decides how a page argues its case. None of them replace the kit: an interface style in particular is not a second source of palettes, fonts, motifs, or brand rules, so apply it through the kit. Each has a list tool returning judgment summaries with no payload and a get tool returning the real export as Markdown by default or JSON on request: list_image_directions and get_image_direction, list_interface_styles and get_interface_style, list_page_recipes and get_page_recipe. Pro records need an entitled key and return 403 without exposing anything.
